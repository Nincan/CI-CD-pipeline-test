"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var tslib_1 = require("tslib");
var jsx_runtime_1 = require("react/jsx-runtime");
var style_1 = require("@sprinklr/spaceweb/style");
var icon_1 = tslib_1.__importDefault(require("../icon"));
function BrandLinkedin(props) {
    var _a = (0, style_1.useStyle)(), theme = _a.theme, isRTL = _a.isRTL;
    return ((0, jsx_runtime_1.jsxs)(icon_1.default, tslib_1.__assign({ viewBox: "0 0 12 11.789", "data-icon-name": "BrandLinkedin" }, props, { children: [(0, jsx_runtime_1.jsx)("path", { d: "M.152 3.834h2.572v7.955H.152zM1.456 0C.575 0 0 .594 0 1.375 0 2.138.557 2.75 1.42 2.75h.019c.896 0 1.455-.612 1.455-1.375C2.878.595 2.335 0 1.456 0zM9.04 3.648c-1.372 0-1.979.775-2.32 1.317v-1.13H4.147c.034.746 0 7.954 0 7.954H6.72V7.348c0-.238.018-.475.086-.645.184-.475.608-.968 1.32-.968.93 0 1.303.73 1.303 1.8v4.254H12V7.23c0-2.443-1.267-3.581-2.96-3.581z" }), (0, jsx_runtime_1.jsx)("path", { d: "M.152 3.834h2.572v7.955H.152zM1.439 2.75h-.018C.557 2.75 0 2.138 0 1.375 0 .595.575 0 1.456 0c.88 0 1.422.594 1.438 1.375 0 .763-.559 1.375-1.455 1.375zM12 11.79H9.429V7.533c0-1.069-.373-1.8-1.304-1.8-.711 0-1.135.494-1.319.969-.068.17-.086.407-.086.645v4.441H4.147s.034-7.208 0-7.955H6.72v1.131c.341-.542.948-1.317 2.32-1.317 1.692 0 2.96 1.138 2.96 3.58v4.561z" })] })));
}
exports.default = BrandLinkedin;
